...
public class DynamoUserDetailsAuthenticationProvider extends AbstractUserDetailsAuthenticationProvider {
	 ...
	 * address the issue mentioned below (Description & Action).
	 * 
	 * This issue was encountered when the 'DynamoUserDetailsAuthenticationProvider'
	 * class was used in a Tulasi Carejoy project.
	 * 
	 *
	 * Description:
	 * 
	 * The bean 'dynamoUserDetailsService' could not be injected as a
		...
		if (authentication != null) {
			...
			if (ud != null && authentication.getCredentials() != null) {
				...
				if (ud.getUsername().equalsIgnoreCase(userEnteredUsername) && encodePassword == true
					...
					logger.error(
							"Username & password in authentication token does not match details in system. Throwing Exception.");
